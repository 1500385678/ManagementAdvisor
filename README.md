...
### Phase 0 资产盘点(7/7 闭环 + 桥接资产 2 块扩展)
...
- [x] 桥接扩展 · 行动项 → 飞书任务映射 · `_rules/行动项转飞书任务映射v0.1.md`(9/15 commit TBD)
...
1. 4.2 §4 真机凭证 6 步接入(预计 9/22 后)
2. 字段映射表在 4 API 真实响应上验证
3. 节点 4 说话人映射 API(directory.user.get)权限补充申请
4. 节点 6 LLM 5 段式抽取 prompt 调试 ✓ (9/14 commit 9a1e288)
5. 节点 8 行动项 → 飞书任务映射表沉淀 ✓ (9/15 commit TBD · `_rules/行动项转飞书任务映射v0.1.md`)
6. dogfood 3 团队周会跑 4 周

### Phase 1 第 1 项前置资产链(5 块齐备)
4.1 输入格式契约 + 4.2 数据来源契约 + 桥接 端到端数据流 + prompt 样例 + 行动项转任务映射 = 5 件套齐备,真机凭证签发后 T2/T3 可直接落地编码
...
- 巡检日志:`.Log/巡检-管理-YYYYMMDD.md`(8/24 立项以来连续 22 天)
